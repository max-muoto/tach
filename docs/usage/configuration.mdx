...
`respect_gitignore` (default: **true**) is a flag which causes Tach to add files excluded by `.gitignore` or global gitignore files to `exclude`.
...
<Note>
  A module can also define `paths` as a shorthand for multiple module definitions.
  This allows specifying allowed dependencies and other attributes as a group.

  Example: `paths = ["a.b", "a.c"]`
</Note>
- `depends_on` a list of the other modules which this module can import from
...
- `visibility` (default: `['*']`) a list of other modules which can import from this module
...
- `unchecked` (default: `false`) marks this module as [**unchecked**](unchecked-modules), meaning Tach will not check its imports
